...
interface GuiaFilters {
  tipo: '' | 'GUIA_REMISION_REMITENTE' | 'GUIA_REMISION_TRANSPORTISTA';
...
export const ComprobanteGuiaSection = () => {
  ...
  const table = useDataTable<ComprobanteSelectDto>({
    data: filteredGuias,
    searchKeys: ['serie', 'numero', 'remitente', 'destinatario', 'transportista'],
    defaultPageSize: 8,
  });
  ...
  const columns: ColumnDef<ComprobanteSelectDto>[] = [
    {
      ...
      width: '170px',
      render: (row) =>
        row.tipo === 'GUIA_REMISION_REMITENTE' ? 'Guía Remitente' : 'Guía Transportista',
    },
    {
      key: 'serie',
      header: 'Serie',
    ...
    { key: 'remitente', header: 'Remitente', sortable: true, render: (row) => row.remitente || '—' },
    { key: 'destinatario', header: 'Destinatario', sortable: true, render: (row) => row.destinatario || '—' },
    {
      key: 'estado',
      header: 'Estado',
      sortable: true,
  ...
  const listContent = (
    <div className="erp-tab-content">
      {error && (
        <div style={{ padding: '8px 12px', marginBottom: '8px', backgroundColor: 'var(--erp-danger-light, #fee2e2)', color: 'var(--erp-danger, #dc2626)', borderRadius: '6px', fontSize: '13px' }}>
          {error}
        </div>
      )}

      {successMessage && (
        <div style={{ padding: '8px 12px', marginBottom: '8px', backgroundColor: 'var(--erp-success-light, #dcfce7)', color: 'var(--erp-success, #16a34a)', borderRadius: '6px', fontSize: '13px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <Toolbar
        ...
        onResetFilters={filterCount ? () => setFilters(INITIAL_FILTERS) : undefined}
        filterPanel={
          <>
            <div className="erp-filter-group">
              <label className="erp-filter-label">Tipo de guía</label>
              <select
                className="erp-filter-select"
                value={filters.tipo}
                onChange={(event) => setFilters((prev) => ({ ...prev, tipo: event.target.value as GuiaFilters['tipo'] }))}
              >
            ...
            <div className="erp-filter-group">
              ...
              <select
                ...
                onChange={(event) => setFilters((prev) => ({ ...prev, estado: event.target.value as GuiaFilters['estado'] }))}
              >
                <option value="">Todos</option>
            ...
            <div className="erp-filter-group">
              ...
              <select
                ...
                onChange={(event) => setFilters((prev) => ({ ...prev, estadoSunat: event.target.value as GuiaFilters['estadoSunat'] }))}
              >
                <option value="">Todos</option>
  ...
  const formContent =
    formMode === 'create' ? (
      <NewGuiaDialog
        ...
        loading={generating}
        guias={comprobantes
          .filter((c) => c.tipo === 'GUIA_REMISION_REMITENTE' || c.tipo === 'GUIA_REMISION_TRANSPORTISTA')
          .map((c) => ({
            id: Number(c.id),
            tipo: c.tipo as 'GUIA_REMISION_REMITENTE' | 'GUIA_REMISION_TRANSPORTISTA',
            serie: c.serie,
            numero: c.numero,
  ...
  return (
    <>
      <SubmoduleTwoTabsLayout
        ...
        subtitle="Remitente y transportista"
        entityName="Guía"
        activeTab={activeTab}
